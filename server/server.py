...
def published_beijing(ts) -> str:
    ...
    跑服务的机器时区不一定是 +8，用 time.localtime 渲染出来的值会跟着机器漂移——
    同一条内容换台机器采就是另一个时间。一律按 +8 渲染，图的是确定性和跨平台一致。

    ⚠️ 各平台页面显示什么时区，并不一致，别照着页面反推：
      B 站 / 小红书  页面显示北京时间，跟本函数的输出对得上
      抖音           页面按**浏览器本机时区**渲染。实测 aweme_id 7677604913714923194
                     真实 epoch 1787591845 = 北京 2026-08-25 01:17:25，而在 PDT 机器上
                     页面显示的是 2026-08-24 10:17——差 15 小时。
    所以取抖音发布时间只能读页面数据对象里的原始 epoch（create_time），
    **绝不能把页面上渲染出来的字符串抠下来当北京时间用**，那会原样搬进一个 15 小时的错，
    正好是 PR #11 要杀的那个 bug 又活一遍。
    """
    try:
        n = int(ts)
